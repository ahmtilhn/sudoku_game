...
patterns = [
    ...
    re.compile(
        r"source = replace_once\(\n"
        r"    source,\n"
        r"    \"      if \(!await _authenticate\(\)\) \{.*?"
        r"    \"connect auth marker\",\n"
        r"\)\n",
        re.S,
    ),
]
for index, pattern in enumerate(patterns, start=1):
    ...
    if count != 1:
        raise RuntimeError(f'expected to remove compatibility block {index}, found {count}')
...
print('Challenge client hardening v3 applied.')
