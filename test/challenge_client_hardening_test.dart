...
void main() {
  test(
    'challenge waiting terminates and room recovery stays challenge-aware',
    () {
      final waiting = File(
  ...
  test('online duel exposes surrender and waits for settlement', () {
    ...
    expect(duel, contains('Future<void> _handoffToRematch'));
    expect(duel, contains('await _controller?.dispose();'));
    expect(duel, contains('_openingAcceptedRoom'));
    expect(prematch, contains('_controller!.ready();'));
    expect(prematch, contains('OnlineDuelStatus.active'));
