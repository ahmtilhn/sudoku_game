...
BEGIN
  ...
  UPDATE players
  SET online_coins = online_coins + 200
  ...
  INSERT OR IGNORE INTO coin_ledger (
    ...
    json_object('pot', 200, 'entryFee', 100),
    NEW.applied_at
  );
...
CREATE TRIGGER escrow_ignore_legacy_winner_update
BEFORE UPDATE OF online_coins ON players
WHEN NEW.online_coins = OLD.online_coins + 100
  AND EXISTS (
    SELECT 1 FROM match_coin_intercepts
    WHERE winner_id = OLD.id AND winner_update_pending = 1
  )
BEGIN
  UPDATE match_coin_intercepts
  SET winner_update_pending = 0
  WHERE winner_id = OLD.id AND winner_update_pending = 1;
  SELECT RAISE(IGNORE);
END;

CREATE TRIGGER escrow_ignore_legacy_loser_update
BEFORE UPDATE OF online_coins ON players
WHEN NEW.online_coins = OLD.online_coins - 100
  AND EXISTS (
    SELECT 1 FROM match_coin_intercepts
    WHERE loser_id = OLD.id AND loser_update_pending = 1
  )
BEGIN
  UPDATE match_coin_intercepts
  SET loser_update_pending = 0
  WHERE loser_id = OLD.id AND loser_update_pending = 1;
  SELECT RAISE(IGNORE);
END;
...
WHEN NEW.status IN ('completed', 'cancelled', 'abandoned')
...
BEGIN
  ...
  UPDATE match_coin_escrow
  SET status = 'refunded', settled_at = COALESCE(NEW.finished_at, NEW.updated_at)
  WHERE match_id = NEW.id AND status = 'funded';
END;
