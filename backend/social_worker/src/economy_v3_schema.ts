export interface EconomyV3SchemaEnv {
  DB: D1Database;
}
...
export async function ensureEconomyV3Schema(env: EconomyV3SchemaEnv): Promise<void> {
  if (installed) return;
  if (installing) return installing;
  installing = install(env).then(() => {
    installed = true;
  }).finally(() => {
    installing = null;
  });
...
async function install(env: EconomyV3SchemaEnv): Promise<void> {
  await env.DB.batch([
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS economy_v3_daily_state (
    ...
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS economy_v3_inventory (
      player_id TEXT PRIMARY KEY,
      hint_refills INTEGER NOT NULL DEFAULT 0 CHECK(hint_refills >= 0),
      updated_at TEXT NOT NULL,
      FOREIGN KEY(player_id) REFERENCES players(id) ON DELETE CASCADE
    )`),
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS economy_v3_inventory_events (
      id TEXT PRIMARY KEY,
    ...
    env.DB.prepare(`CREATE TRIGGER IF NOT EXISTS economy_v3_inventory_event_apply
      ...
      END`),
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS economy_v3_coin_events (
      id TEXT PRIMARY KEY,
      player_id TEXT NOT NULL,
      source TEXT NOT NULL,
      reference_id TEXT NOT NULL,
      ...
      ledger_reason TEXT NOT NULL CHECK(ledger_reason IN (
        'daily_login','daily_rewarded_ad','career_rewarded_ad','achievement_reward','career_continue'
      )),
    ...
    env.DB.prepare(`CREATE TRIGGER IF NOT EXISTS economy_v3_coin_event_balance_guard
      ...
      BEGIN
        ...
        END;
      END`),
    env.DB.prepare(`CREATE TRIGGER IF NOT EXISTS economy_v3_coin_event_apply
      ...
      END`),
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS economy_v3_career_progress (
      player_id TEXT NOT NULL,
    ...
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS economy_v3_career_daily (
      player_id TEXT NOT NULL,
    ...
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS economy_v3_recovery_daily (
      player_id TEXT NOT NULL,
      day_key TEXT NOT NULL,
      coins_earned INTEGER NOT NULL DEFAULT 0 CHECK(coins_earned >= 0),
    ...
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS economy_v3_recovery_matches (
      player_id TEXT NOT NULL,
