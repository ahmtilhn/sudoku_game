...
-- Visible competitive progression is deliberately separated from the existing
-- 1000-based Elo/MMR tables. Existing Elo remains the matchmaking skill signal.
...
CREATE INDEX IF NOT EXISTS player_rank_progression_points_idx
  ON player_rank_progression(rank_points DESC, ranked_games DESC, updated_at ASC, player_id ASC);

-- Exactly-once audit of visible RP settlement. This table is intentionally
-- independent from match_settlements so the authoritative online duel and Elo
-- settlement do not need to be modified.
CREATE TABLE IF NOT EXISTS rank_progression_settlements (
...
CREATE INDEX IF NOT EXISTS rank_progression_pair_recent_idx
  ON rank_progression_settlements(player_id, opponent_id, finished_at DESC);
CREATE INDEX IF NOT EXISTS rank_progression_player_time_idx
  ON rank_progression_settlements(player_id, finished_at ASC);

-- Rank milestone rewards are lifetime-first-time rewards. The INSERT is the
-- idempotency boundary; the trigger atomically applies the Coin grant and ledger
-- record without touching match escrow or match payout logic.
CREATE TABLE IF NOT EXISTS rank_reward_grants (
  player_id TEXT NOT NULL,
...
BEGIN
  ...
  INSERT OR IGNORE INTO coin_ledger (
    ...
    json_object('source', 'rank_progression', 'rankKey', NEW.rank_key, 'lifetimeFirstTime', 1),
    NEW.granted_at
  );
END;

-- Achievement definitions can expose a cosmetic attachment for the player's
-- frame. achievement_showcase remains the authoritative three-slot selection.
...
INSERT OR IGNORE INTO achievement_definitions (
  ...
  ('undefeated_10', 'ranked', 'Unbeaten 10', 'Finish 10 ranked duels in a row without a loss.', 'silver', 0, NULL, 0, 120),
  ('undefeated_25', 'ranked', 'Unbeaten 25', 'Finish 25 ranked duels in a row without a loss.', 'gold', 0, NULL, 0, 130),
  ('undefeated_50', 'ranked', 'Unbeaten 50', 'Finish 50 ranked duels in a row without a loss.', 'milestone', 0, NULL, 0, 140),
  ('win_streak_5', 'ranked', 'Five Win Streak', 'Win 5 ranked duels in a row.', 'bronze', 0, NULL, 0, 150),
  ('win_streak_10', 'ranked', 'Ten Win Streak', 'Win 10 ranked duels in a row.', 'gold', 0, NULL, 0, 160),
  ('win_streak_25', 'ranked', 'Twenty Five Win Streak', 'Win 25 ranked duels in a row.', 'milestone', 0, NULL, 0, 170),
  ('rank_silver', 'season', 'Silver Competitor', 'Reach Silver III for the first time.', 'silver', 0, NULL, 0, 180),
  ('rank_gold', 'season', 'Gold Competitor', 'Reach Gold III for the first time.', 'gold', 0, NULL, 0, 190),
  ('rank_platinum', 'season', 'Platinum Competitor', 'Reach Platinum III for the first time.', 'platinum', 0, NULL, 0, 200),
  ('rank_master', 'season', 'Master Competitor', 'Reach Master III for the first time.', 'milestone', 0, NULL, 0, 210),
  ('rank_master_i', 'season', 'Master I', 'Reach Master I for the first time.', 'milestone', 0, NULL, 0, 220),
  ('giant_slayer', 'ranked', 'Giant Slayer', 'Defeat a ranked opponent at least 251 MMR above you.', 'gold', 0, NULL, 0, 230),
  ('ranked_veteran_100', 'ranked', 'Ranked Veteran', 'Finish 100 ranked duels.', 'silver', 0, NULL, 0, 240),
  ('ranked_veteran_500', 'ranked', 'Elite Veteran', 'Finish 500 ranked duels.', 'platinum', 0, NULL, 0, 250),
  ('ranked_veteran_1000', 'ranked', 'Legendary Veteran', 'Finish 1000 ranked duels.', 'milestone', 0, NULL, 0, 260),
  ('perfect_ranked_win', 'sudoku', 'Perfect Duel', 'Win a ranked duel without a mistake or timeout.', 'silver', 0, NULL, 0, 270),
  ('perfect_ranked_wins_10', 'sudoku', 'Perfect Ten', 'Win 10 ranked duels without a mistake or timeout.', 'platinum', 0, NULL, 0, 280);

INSERT OR IGNORE INTO achievement_cosmetics (achievement_id, decoration_key, rarity, sort_order) VALUES
...
-- Migration-time baseline: every account that already exists starts the new
-- visible ladder at exactly 0 RP. Old Elo history is not converted into RP.
...
SELECT
  ...
  0,
  CASE
    WHEN avatar_key LIKE 'idv1|%' THEN 'default'
    ELSE COALESCE(NULLIF(avatar_key, ''), 'default')
  END,
  'auto',
  '',
